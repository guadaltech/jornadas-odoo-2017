...
if [ $PS_THEME = "0" ]; then
	echo "* Copia inicial PS original <>"
	sudo cp -R /tmp/mysql.org/* /var/lib/mysql/
fi
if [ $PS_THEME = "1" ]; then
	echo "* Copia inicial PS con tema"
	sudo cp -R /tmp/mysql.theme/* /var/lib/mysql/
fi
...
mysqladmin -u root -p'xxxxxxxxx' password $MYSQL_ROOT_PASSWORD
